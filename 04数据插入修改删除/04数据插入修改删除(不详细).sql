1.插入操作
    ...
    insert into mb_int4 values(1,"aaa"); //不写字段列表也可以
    insert into mb_int4(id,name) values(2,"bbb");
    insert into mb_int4(name,id) values("bbb",2); //这样可以,前后一致即可
    insert into mb_int4(name) values("bbb");      //不全写也可以
...
4.查询操作
    ...
    (3) 高级查询
        select sex,age+1 from mb_students1; //年龄+1显示
    ...
    (5) group by 分组查询
        ...
        select sex,count(*) from mb_students1 group by sex;
    ...
    (6) 回溯统计
            ...
            select class,cout(*) from mb_students1 group by class with rollup;
            class count(*)
            1     4
            2     5
            3     4
    ...
    (7) 排序
        select * from mb_students1 order by score;  默认升序
                                         by score desc; 降序
                                         by score asc;  升序

    (8) where
        select * from mb_students1 where score = 60; mysql中没有 == 号,用 = 就可以
        select * from mb_students1 where score > 59;

    (9) having
        select * from mb_students1 having score > 59; //这一行和上面的where效果相同
    ...
    (10) 模糊查询 like _ %
        _ 表示一个字符
        % 表示0或者多个字符
        select * form mb_students1 where name like "李%";

    (11) limit 限制
        select * from mb_students1 limit a,b; 从第a+1条开始,显示b条
        select * from mb_students1 limit b;   从第1条开始,显示b条

        offset 关键字
        select * from mb_students1 a offset b; 从b+1条开始,输出a条

    (12) distinct 不显示重复项
        select distinct age from mb_students1; 只显示单个的age
        select id,distinct age ... //这样写是错误的

    (13) in在where中规定多个值
        select * from mb_students1 where class in(1,2); 显示1班和2班的同学

    (14) [not] between and 在where中规定范围
        select * from mb_students1 where score between 60 and 100; 60~100之间,包括这两个数字
        select * from mb_students1 where score not between 60 and 100; 不在60~100之间

    (15) and  or
        select * from mb_students1 where class in (1,2) and score between 60 and 100;
        select * from mb_students1 where class in (1,2) or score between 60 and 100;
    (16)is / is not
    ...
    (17) as别名 as关键字可以省略
        select id as "学号" , name as "姓名" from mb_students1 as 班级表; 最后的表的别名不能加引号
        学号  姓名
        1     张三
        2     李四
        ...
    ...
    (18) concat concat_ws 连接前后
        select id,concat(name,',',sex,',',age) as "基本信息" from mb_students1 ;
                        (name,',',sex,',',age) ','的作用是在下面输入的时候当做分隔符
        select id,concat_ws(':',name,sex,age) as "基本信息" from mb_students1 ;
                           (':',name,sex,age)  ':' 就是下面输入的分隔符

    (19)union unionall 联合两个表显示
        union不会显示重复的
        unionall会全部显示
        select id,name from mb_students1 union select id ,name from mb_students2;
        select id,name from mb_students1 union all select id ,name from mb_students2;  //后面的id,name顺序可以互换,但是不建议
    ...
    (21) ***
        ...
        select class,sex,count(*) from mb_students1 where class < 10 group by class,sex order by class;