...
接下来，我们来做一个测试：
    ...
    B. 执行排序SQL
        explain select id,age,phone from m_tb_user order by age ;

        explain select id,age,phone from m_tb_user order by age, phone ;
    ...
    E. 创建索引后，根据age, phone进行降序排
        explain select id,age,phone from m_tb_user order by age desc , phone desc ;
    ...
    F. 根据phone，age进行升序排序，phone在前，age在后。
        explain select id,age,phone from m_tb_user order by phone , age
    ...
    G. 根据age, phone进行降序一个升序，一个降序
        explain select id,age,phone from m_tb_user order by age asc , phone desc ;
