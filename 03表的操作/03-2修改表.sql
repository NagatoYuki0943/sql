1.设置表选项
    ...
    alter table int3 charset=gbk

    注意:如果数据库已经有数据了, 不要轻易修改表选项

2.修改表结构
    (1).修改表名:   rename table 旧表名 to 新表名
        rename table students6_1 to students6;

    (2).新增列:     alter table 表名 add [column] 新列名 列类型 [列属性] [位置first,after]
        默认加到表的最后面,first放到最前面,after+字段名,放到谁的后面
        alter table students6 add birthday date comment '生日';

    (3).修改列属性: alter table 表名 modify 列名 新类型 [新属性] [新位置]
        alter table students6 modify name varchar(100);

    (4).修改列名:   alter table 表名 change 旧列名 新列名 列类型 [列属性] [新位置]
        alter table students6 change gender sex enum('male', 'female', 'futa') default 'futa' comment '性别';

    (5).删除列:     alter table 表名 drop 列名
        alter table students6 drop birthday1;
...
4.添加主键/unique
    添加:
        ...
        alter table score add primary key(key1,key2);  //多个组成复合主键,删除一次全部删除
        alter table 表名 add unique(id);
        alter table 表名 add unique key(id); 和上一行相同
        alter table unique1 add unique(key1,key2); //可以一次添加多个
