...
4 timestamp
    ...
    alter table date modify d4 timestamp not null default current_timestamp on update current_timestamp;
    数据被修改时,时间戳会自动更新
...
8 数据被修改时,时间戳会自动更新
    update date set d1='2000-01-01' where d5=2069;
    +------------+----------+---------------------+---------------------+------+
    | d1         | d2       | d3                  | d4                  | d5   |
    +------------+----------+---------------------+---------------------+------+
...
10 查询时间 需要加引号
    select * from m_emp where entrydate > '2008-12-12';
    +----+--------+-----+------+--------+------------+-----------+---------+
