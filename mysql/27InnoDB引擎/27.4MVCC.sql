1 基本概念
    ...
    测试：
        ...
        右侧:
            begin;
            update m_stu set name='Jsp' where id=1;
        ...
        左侧:
            ...
            | id | name  | age |
            +----+-------+-----+
            |  1 | Java  |   1 |    --右侧提交后还是看不到,保证了可重复读,不是当前读
            |  3 | PHP   |   3 |
            |  7 | BOB   |   7 |
            |  8 | rose  |   8 |
            | 11 | jetty |  11 |
            ...
            select * from m_stu lock in share mode;
            +----+-------+-----+
            | id | name  | age |
            +----+-------+-----+
            |  1 | Jsp   |   1 |    -- lock in share mode 就能读到最新的数据了,是当前读(读取最新的数据)
            |  3 | PHP   |   3 |
            |  7 | BOB   |   7 |
            |  8 | rose  |   8 |
            | 11 | jetty |  11 |
    ...
    2). 快照读
        ...
        不加锁，是非阻塞读。
            ...
            • Repeatable Read(默认)： 开启事务后第一个select语句才是快照读的地方。
            • Serializable：          快照读会退化为当前读。

        在上面的测试中,我们看到即使事务B提交了数据,事务A中也查询不到。 原因就是因为普通的select是快照
